...
overlays.forEach((o, i) => {
  ...
  chain += `[${prev}][${i + 1}:v]overlay=x=(W-w)/2:y=H*0.05-60:enable='between(t,${o.start.toFixed(3)},${o.end.toFixed(3)})'[${label}];`;
  prev = label;
});
chain = chain.slice(0, -1);

const cmd = `ffmpeg -y -i "usecase-news.mp4" ${inputs} -filter_complex "${chain}" -map "[vout]" -c:v libx264 -crf 18 -preset slow -movflags +faststart "/tmp/news_captioned.mp4"`;
